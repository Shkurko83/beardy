...
struct HTMLVisitor: MarkupVisitor {
    ...
    private func resolveImageSource(_ rawSrc: String) -> String {
        if rawSrc.hasPrefix("data:")
            ...
            || ImageInsertionHelper.isCustomImageURL(rawSrc) {
            return rawSrc
        }

        if rawSrc.hasPrefix("file://") {
            let filePath = rawSrc.replacingOccurrences(of: "file://", with: "")
            return ImageInsertionHelper.blackBeardURL(forLocalPath: filePath)
        }

        if rawSrc.hasPrefix("/") {
            return ImageInsertionHelper.blackBeardURL(forLocalPath: rawSrc)
        }
        ...
        if let docURL = documentURL {
            ...
            return ImageInsertionHelper.blackBeardURL(forLocalPath: absolutePath)
        }
