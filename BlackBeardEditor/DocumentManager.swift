...
class DocumentManager: ObservableObject {
    ...
    func closeTab(_ id: UUID) {
        ...
        if tab.document.hasUnsavedChanges {
            switch promptForUnsavedTab(tab, warnIfEnabled: true) {
            case .save:
            ...
            case .cancel:
                return
            case .dontSave:
                RecoveryBackupStore.remove(document: tab.document)
            }
        }

        recoveryWriteWorkItems.removeValue(forKey: id)
    ...
    private func promptForUnsavedTab(_ tab: EditorTab, warnIfEnabled: Bool) -> UnsavedCloseChoice {
        guard tab.document.hasUnsavedChanges else { return .dontSave }
        if warnIfEnabled && !AppConstants.warnBeforeClosingUnsaved {
            return .dontSave
        }
        return showUnsavedCloseAlert(for: tab)
    }

    private func showUnsavedCloseAlert(for tab: EditorTab) -> UnsavedCloseChoice {
        let alert = NSAlert()
    ...
    func performStartupActionIfNeeded() {
        guard !Self.hasPerformedStartupAction else { return }
        Self.hasPerformedStartupAction = true
        guard tabs.isEmpty else { return }

        switch AppConstants.startupAction {
        case .welcome:
            offerRecoveryBackupsIfNeeded()
        ...
        case .last:
            if AppConstants.restoreOpenFilesOnLaunch, restoreSessionIfNeeded() {
                offerRecoveryBackupsIfNeeded()
                return
            }
            offerRecoveryBackupsIfNeeded()
            guard tabs.isEmpty else { return }
            openLastDocumentOnStartup()
        }
    }
    ...
    private func restoreTabFromSession(_ saved: SessionStore.SavedTab) {
        if let path = saved.path {
            let resolved = SecurityBookmarkStore.resolveURL(path: path) ?? URL(fileURLWithPath: path)
            Self.grantAccessAndSaveBookmarks(for: resolved)
            guard let content = try? String(contentsOf: resolved, encoding: .utf8) else { return }
    ...
    private func offerRecoveryBackupsIfNeeded() {
        let pending = RecoveryBackupStore.pendingRecoveries()
        guard !pending.isEmpty else { return }

        let formatter = DateFormatter()
        formatter.dateStyle = .medium
        formatter.timeStyle = .short
        let list = pending.map { item in
            "• \(item.entry.fileName) (\(formatter.string(from: item.entry.savedAt)))"
        }.joined(separator: "\n")

        let alert = NSAlert()
        alert.messageText = "Recover unsaved changes?"
        alert.informativeText = """
        Black Beard Editor found recovery copies from a previous session:

        \(list)

        Recovery files are stored in Application Support and removed after you save.
        """
        ...
        case .alertSecondButtonReturn:
            for item in pending {
                RecoveryBackupStore.remove(recoveryKey: item.entry.recoveryKey)
            }
        default:
            break
        }
    ...
    private func applyRecovery(_ item: (entry: RecoveryBackupStore.Entry, content: String)) {
        if let path = item.entry.originalPath {
            let normalized = URL(fileURLWithPath: path).standardizedFileURL.path
            if let index = tabs.firstIndex(where: { $0.document.url?.standardizedFileURL.path == normalized }) {
                tabs[index].document.content = item.content
                tabs[index].document.hasUnsavedChanges = true
                tabs[index].document.lastModifiedDate = Date()
                tabs[index].document.updateStatistics()
                if selectedTabID == tabs[index].id {
                    execEditorJS("window.cmEditor?.updateContent(`\(escapeForRecoveryJS(item.content))`);")
                }
                return
            }

            if let url = SecurityBookmarkStore.resolveURL(path: path) {
        ...
        var doc = MarkdownDocument(fileName: item.entry.fileName, content: item.content)
        doc.hasUnsavedChanges = true
        openInNewTab(doc)
    }

    private func escapeForRecoveryJS(_ value: String) -> String {
        value
    ...
    func handleApplicationTerminate() -> NSApplication.TerminateReply {
        persistSessionState()

        guard AppConstants.warnBeforeClosingUnsaved else { return .terminateNow }

        for tab in tabs where tab.document.hasUnsavedChanges {
            switch showUnsavedCloseAlert(for: tab) {
            case .save:
                let previous = selectedTabID
                selectedTabID = tab.id
                saveDocument()
    ...
    private func scheduleRecoveryBackup(for tabID: UUID) {
        guard AppConstants.isRecoveryBackupEnabled else { return }
        recoveryWriteWorkItems[tabID]?.cancel()
        let work = DispatchWorkItem { [weak self] in
            self?.writeRecoveryBackup(for: tabID)
        }
        recoveryWriteWorkItems[tabID] = work
        DispatchQueue.main.asyncAfter(deadline: .now() + 2.0, execute: work)
    }

    private func writeRecoveryBackup(for tabID: UUID) {
        guard AppConstants.isRecoveryBackupEnabled,
              let tab = tabs.first(where: { $0.id == tabID }),
              tab.document.hasUnsavedChanges else { return }
        RecoveryBackupStore.write(document: tab.document, content: tab.document.content)
    }

    private func openLastDocumentOnStartup() {
        if let path = UserDefaults.standard.string(forKey: AppConstants.Keys.lastOpenedDocument),
           let url = SecurityBookmarkStore.resolveURL(path: path) {
            openDocument(at: url, inNewTab: true)
            return
        }
        if let recent = recentDocuments.first {
            openRecentDocument(recent)
        }
    }

    private func persistLastOpenedDocument(path: String) {
        UserDefaults.standard.set(path, forKey: AppConstants.Keys.lastOpenedDocument)
    }
    ...
    func showImageInsertDialog(url: URL) {
        ...
        if let docURL = currentDocument?.url {
            Self.grantAccessAndSaveBookmarks(for: docURL)
            DocumentSecurityAccess.activate(document: docURL)
        }
    ...
    private func autoSave() {
        guard UserDefaults.standard.object(forKey: AppConstants.Keys.autoSaveEnabled) as? Bool ?? true else {
            return
        }
        let interval = UserDefaults.standard.object(forKey: AppConstants.Keys.autoSaveInterval) as? Double
            ?? AppConstants.Defaults.autoSaveInterval
        let now = Date()
        guard now.timeIntervalSince(lastAutoSaveCheck) >= interval else { return }
