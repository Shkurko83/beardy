...
enum SnapshotStore {
    ...
    static func deleteSidecar(for documentURL: URL) {
        ...
        _ = try? SecurityBookmarkStore.withDocumentFolderAccess(documentURL: documentURL) {
            try? FileManager.default.removeItem(at: sidecar)
        }
    ...
    static func promptDeleteSidecarIfNeeded(for documentURL: URL) {
        let sidecar = documentURL.deletingPathExtension().appendingPathExtension("snapshots.json")
        let exists = (try? SecurityBookmarkStore.withDocumentFolderAccess(documentURL: documentURL) {
    ...
    private static func withDocumentFolderAccess<T>(for document: MarkdownDocument, _ work: () -> T) -> T {
        ...
        if let result = try? SecurityBookmarkStore.withDocumentFolderAccess(documentURL: url, work) {
            return result
        }
        return work()
    }
