...
private struct DocxDocumentBuilder: MarkupVisitor {
    ...
    private func resolveImageFileURL(_ rawSrc: String?) -> URL? {
        ...
        if src.hasPrefix("beardy://"), let url = URL(string: src),
           let path = ImageInsertionHelper.localPath(fromBeardyURL: url) {
    ...
    mutating func write(to outputURL: URL) throws {
        ...
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("beardy-docx-\(UUID().uuidString)", isDirectory: true)
    ...
    private func coreProperties(title: String) -> String {
        ...
        <dc:creator>Beardy2</dc:creator>
        <cp:lastModifiedBy>Beardy2</cp:lastModifiedBy>
...
private enum DocxPackageTemplates {
    ...
    <Application>Beardy2</Application>
    </Properties>
    """
}
